...
def plot(L, scale=4, dot_size=3):
    """ plot takes a list of points, optionally a scale (relative to a 200x200 frame),
        ...
        It produces an SVG string representing the given plot, and returns it.
    """
    scalar = 200. / scale
    origin = (210, 210)
    svg = ['<svg height="420" width="420" xmlns="http://www.w3.org/2000/svg">'
    ...
    for pt in L:
        if isinstance(pt, Number):
            ...
            if isinstance(pt, tuple) or isinstance(pt, list):
                ...
                raise ValueError
        svg.append('<circle cx="%d" cy="%d" r="%d" fill="red"/>'
